...
// bool  Mse( Eigen::Matrix<float, Eigen::Dynamic, 3>& P, Circle& circle_hat)
//     /*
//     This function Mse computes the mean square error given data and estimation. MSE is determined by averaging the
//     squared root of differences between the distances of each data point to the estimated circle center and the estimated radius. 

//     The return type is bool so a possible usage is :
//     if (Mse < threshold ): estinmation accept, push circle into container
//     else : data abandoned
//     */
// {
//     // create a vector x y z 
//     // propogate the vecotr along y axis
//     // get the difference
//     // square each element and sum up along colum axis
//     // root then average along the row axis

//     Eigen::Matrix<float, 1, 3> center =(circle.Px, circle.Py, circle.Pz);
//     //Diff_p_c = P.rowwise() - center;
//     Eigen::Matrix<float, Eigen::Dynamic, 1> Diff_p_c = ( P.rowwise() - center).array().square().rowwise().sum();  //<n , 1>
//     float MSE = Diff_p_c.array().root().colwise().mean();



// }

// void computeMSE(const Eigen::Matrix<float, Eigen::Dynamic, 3>& dataPoints,  Circle& estimatedCircle)
// {
//     /*
...
//         estimatedCircle (const Circle&): The estimated circle represented by its properties (Px, Py, Pz) as the center.

//     */

//     // Calculate the center of the estimated circle.
//     Eigen::Vector3f center(estimatedCircle.Px, estimatedCircle.Py, estimatedCircle.Pz);

//     // Compute the squared differences between data points and the circle center.
...
//     float computedMSE = squaredDistances.array().sqrt().mean();

//     Circle.s = computedMSE;
//     // return (computedMSE < MSE_THRESHOLD);
// }
...
{   
    ...
    Eigen::Matrix<float, Eigen::Dynamic, 3> P_ori = P; // original P is modified by Centralization, so save one for computing MSE later
...
*/  
    ...
    float test_npx_3[]  {1.13, 1.21, 1.06, 0.83, 0.59, 0.95, 0.59, 0.54, 0.23, 0.09, -0.11, -0.21, -0.27, -0.65, -0.84, -0.99, -1.22, -1.38, -1.84, -2.15};
    float test_npy_3[]  {4.81, 5.28, 5.35, 5.68, 5.78, 6.36, 6.37, 6.51, 6.87, 6.86, 7.08, 7.44, 7.71, 7.83, 7.95, 7.97, 8.07, 8.09, 8.01, 8.05};
    float test_npz_3[]  {5.02, 5.10, 5.27, 5.21, 5.07, 5.01, 5.07, 5.06, 4.91, 4.72, 4.84, 4.60, 4.63, 4.28, 4.28, 4.24, 4.15, 3.82, 3.71, 3.49};
    

    Eigen::Matrix<float, Eigen::Dynamic, 3> P_test_2; // Dynamic-sized matrix for points
...
*/  
    ...
    Eigen::Matrix<float, Eigen::Dynamic, 3> P_test_3; // Dynamic-sized matrix for points
...
*/  
    ...
    Eigen::Matrix<float, Eigen::Dynamic, 3> P_test_4; // Dynamic-sized matrix for points
