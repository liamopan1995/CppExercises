...
int main() {
    ...
    float temperature = 0;
    float windChill = 0;
    cout<<" Wind chill as a function of temperature and wind speed  displayed as table:"<<endl<<endl;
    ...
    cout<<endl;
    for(float j=5.0;j<=60;j+=5){//  j is windspeed iteration
    for (float i = 40.0; i>=-45; i-=5) // i is temperature iteration
    {   
        cout<<"  ";
        computewindChill(j,i,windChill);
  ...
  void computewindChill(float windSpeed,float temperature,float & windChill)
{
    ...
    windChill = (windChill>=0)? (int(windChill+0.5)):(int(windChill-0.5));
}
